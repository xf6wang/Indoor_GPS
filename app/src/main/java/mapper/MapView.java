...
public class MapView extends View
{
    ...
    private List<LabeledPoint> compassCoord = new ArrayList<LabeledPoint>();
    ...
    public void addCompassValue(PointF center, PointF Line)
    {
        compassCoord.clear();
        //add the center point
        LabeledPoint ret = new LabeledPoint(center, "");
        compassCoord.add(ret);
        //add the compass pointer
        ret = new LabeledPoint(Line, "");
        compassCoord.add(ret);
        invalidate();
    }
    ...
    protected void onDraw(Canvas canvas){
        ...
        //draw out the compass coordinates
        // for(LabeledPoint lp : compassCoord){

        //draw out the center
        if (compassCoord.size() > 0) {
            PointF p = ((LabeledPoint) compassCoord.get(0)).getPoint();
            canvas.drawCircle(p.x * scale.x, p.y * scale.y, 1, linePaints.get(LABELlED_POINT_COLOR_INDEX));
            //canvas.drawText("Compass", 2 + p.x * scale.x, p.y * scale.y, linePaints.get(LINE_COLOR_INDEX));
            PointF p2 = ((LabeledPoint) compassCoord.get(1)).getPoint();
            canvas.drawCircle(p2.x * scale.x, p2.y * scale.y, 10, linePaints.get(LABELlED_POINT_COLOR_INDEX));
            //canvas.drawText("", 2 + p2.x * scale.x, p2.y * scale.y, linePaints.get(LINE_COLOR_INDEX));

            canvas.drawLine(p.x * scale.x, p.y * scale.y, p2.x * scale.x, p2.y * scale.y,
                    linePaints.get(LINE_COLOR_INDEX));
        }
